...
Microsoft.Maps.Pushpin.prototype.ShowInfobox = function(speed){
	if (!this._infoboxShown && this._infoboxWrapper){
		speed = speed || 'fast';
...
Microsoft.Maps.Pushpin.prototype._click = function(e){
	...
	if (entity){
		if (entity.GetInfoboxShown()){
			if (!entity._autosticky && !entity._sticky){
...
Microsoft.Maps.EntityCollection.prototype._entityChanged = function(e){
	...
	if (entity instanceof Microsoft.Maps.Pushpin){
		...
		if (entity._infobox){
		...
		} else if (entity._title || entity._description){
			...
			var title = (entity._title ? entity._title : '');
			if (entity._infoboxTitle){
				entity._infoboxTitle.html(title);
			} else {
				entity._infoboxTitle = jQuery('<div class="Title">' + title + '</div>');
				entity._infoboxWrapper.append( entity._infoboxTitle );	
			}

			var description = (entity._description ? entity._description : '');
			if (entity._infoboxDescription){
				entity._infoboxDescription.html(description);
			} else {
				entity._infoboxDescription = jQuery('<div class="Description">' + description + '</div>');
